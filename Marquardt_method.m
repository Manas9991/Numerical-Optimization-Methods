res = 0.000001; %resolution of solution (allowable error)
...
while (abs(X1-X0) >= res)
    ...
    yy = [yy x0(2)];
    if (iterations>3) 
        sk = -1*inv(hessian(f, [x, y]))*gradient(f, [x, y]);
        s1 = subs(sk, {x, y}, {x1(1), x1(2)}); %Gradient at next iteration's initial point
    else
        s1 = subs(grad, {x, y}, {x1(1), x1(2)});
    end
